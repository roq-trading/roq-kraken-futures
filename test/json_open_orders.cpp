...
#include "roq/kraken_futures/protocol/json/open_orders.hpp"
...
TEST_CASE("json_open_orders_new_placed_order_by_user", "[json_open_orders]") {
  ...
  protocol::json::OpenOrders obj{message, buffer};
  CHECK(obj.feed == protocol::json::Feed::OPEN_ORDERS);
  ...
  CHECK(obj.order.type == protocol::json::OrderType::LIMIT);
  ...
  CHECK(obj.reason == protocol::json::Reason::NEW_PLACED_ORDER_BY_USER);
}
...
TEST_CASE("json_open_orders_edited_by_user", "[json_open_orders]") {
  ...
  CHECK(obj.order.reduce_only == false);
  CHECK(obj.is_cancel == false);
  CHECK(obj.reason == protocol::json::Reason::EDITED_BY_USER);
}
...
TEST_CASE("json_open_orders_cancelled_by_user", "[json_open_orders]") {
  ...
  CHECK(obj.reason == protocol::json::Reason::CANCELLED_BY_USER);
}
...
TEST_CASE("json_open_orders_full_fill", "[json_open_orders]") {
  ...
  CHECK(obj.reason == protocol::json::Reason::FULL_FILL);
}
