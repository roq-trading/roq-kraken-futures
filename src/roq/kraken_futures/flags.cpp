...
ABSL_FLAG(std::string, rest_uri, "https://api.kraken.com/0", "REST end-point (URI)");

ABSL_FLAG(std::string, ws_uri, "wss://beta-ws-auth.kraken.com", "WebSocket end-point (URI)");
...
ABSL_FLAG(std::string, exchange, "kraken-futures", "exchange identifier (string)");
...
ABSL_FLAG(uint32_t, book_depth, 0, "book depth (0 == exchange default)");
...
uint32_t Flags::book_depth() {
  static const uint32_t result = absl::GetFlag(FLAGS_book_depth);
  return result;
}
